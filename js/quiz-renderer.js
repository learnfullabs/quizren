(function (Drupal, drupalSettings) {
  ...
  Drupal.behaviors.quizRenderer = {
    attach: function (context, settings) {
      ...
      this.fetchQuizData(nid, apiEndpoint)
        .then(data => {
          ...
          if (data && data.length > 0) {
            ...
            if (quizData && quizData.length > 0) {
              ...
              // Node exists but no quiz data yet - show generation message
              this.showQuizGenerating(renderDiv);
            }
          } else {
            // Node exists but no quiz data yet - show generation message
            this.showQuizGenerating(renderDiv);
        ...
        .catch(error => {
          ...
          // Check if it's a 404 or other HTTP error vs network error
          if (error.message && error.message.includes('404')) {
            this.showError(renderDiv, 'Quiz not found. Please check the quiz ID.');
          } else if (error.message && error.message.includes('HTTP error!')) {
            this.showError(renderDiv, 'Failed to load quiz data. Please try again later.');
          } else {
            this.showError(renderDiv, 'Unable to connect to quiz service. Please check your connection and try again.');
          }
        });
    },
    ...
    /**
     * Shows a quiz generation message using Bootstrap placeholders.
     */
    showQuizGenerating: function(outputDiv) {
      outputDiv.innerHTML = `
        <div class="container-fluid">
          <div class="alert alert-info" role="alert">
            <div class="d-flex align-items-center">
              ...
              <div>
                <strong><i class="bi bi-gear-fill me-1"></i>Quiz Being Generated</strong>
                <p class="mb-0 mt-1">Your quiz is currently being generated by AI. This usually takes 1-2 minutes. Please refresh the page in a moment.</p>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <span class="placeholder col-3 btn btn-outline-secondary disabled"></span>
              </div>
            </div>
          </div>
